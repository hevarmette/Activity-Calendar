...
export function DetailMap({
  ...
  const autoLapDist =
    autoLapDistProp ??
    AUTO_LAP_DISTANCES[sport] ??
    AUTO_LAP_DISTANCES["default"]!;
  ...
  return (
    <div ref={containerRef} className="relative h-full w-full">
      <button
        ...
        className="absolute bottom-8 right-2 z-[1000] rounded-md bg-white border border-gray-300 shadow-md px-2 py-1.5 text-gray-700 hover:bg-gray-100 transition-colors text-sm leading-none"
      >
        {isFullscreen ? "⤓" : "⤢"}
      </button>
      <MapContainer
        ...
        scrollWheelZoom
        preferCanvas
        className={`${isFullscreen ? "h-full" : "h-[500px]"} w-full rounded-lg`}
      >
        <LayersControl position="topright">
                  ...
                  {sessions.map((session) => {
                    ...
                    return displaySeg.length > 1 ? (
                      <Polyline
                        ...
                        pathOptions={{
                          color: SPORT_COLORS[session.sport] ?? "#7F7F7F",
                          weight: 5,
                        }}
                      />
                    ) : null;
              ...
              <LayersControl.Overlay name="Default Line Color">
                <FeatureGroup>
                  <Polyline
                    ...
                    pathOptions={{ color: "#FF4B4B", weight: 5 }}
                  />
                </FeatureGroup>
              </LayersControl.Overlay>
            </>
          ...
          <LayersControl.Overlay
            checked={showAutoMilesByDefault}
            name="Auto Mile Markers"
          >
        ...
        {hoveredIndex != null && points[hoveredIndex]?.latitude != null && (
          <CircleMarker
            center={[
              points[hoveredIndex]!.latitude!,
              points[hoveredIndex]!.longitude!,
            ]}
            radius={6}
            pathOptions={{
              color: "#dc2626",
