...
- [x] **E2E tests with Playwright** — Config + test files covering: calendar navigation, activity detail view + save, search + filter, report grouping.
  ...
  ***
  ...
  - [x] activity report is not summing manual activities made in garmin connect
  - [x] **Manual/session-less activity export distance** — In `activity-fit.ts`, the fallback SESSION (activities with no session row) seeded `totalDistance` from `totalTimerTime` (a duration). Fixed: fallback distance now sums the recorded lap distances. Verified via encode→decode round-trip (session totalDistance = 500 from laps, not the 300s timer).
  - [x] **Activity search "Select all" button not working** — Toggling "Select all" doesn't persist the selection. `allMatchingIds` is a fresh array reference each render, and the `useEffect([allMatchingIds])` that resets `selectedIds` fires right after the toggle, clearing it. Fixed by resetting on a stable `matchingIdsSignature` (joined id string) instead of the array identity, so the selection only clears when the matching set actually changes by value.
  - [x] **Activity search date field resets while typing the year** — The `type="date"` from/to inputs called `updateParam` on every `onChange`, committing partial/invalid dates to the URL (and resetting `page`), so mid-typing a year (e.g. "202" before "2026") wiped the value. Fixed: from/to inputs now use local state and only commit to the URL when the value is a complete valid date (via `isCommittableDate`), or on blur; invalid intermediate values are ignored.
  - [x] **Allow search with no criteria (browse all)** — `useSearch` always fetches (the server `/api/search` route returns the full activity list ordered by timestamp desc when no text params are given), and the results panel now renders whenever `activities.length > 0` instead of gating on `hasFilters`. Client-side date/sport/category/distance/duration filters and sort apply on top, so browsing the full library and filtering without a text query both work.
  - [x] Allow to click link in the activity comparison page. Show how far behind over time an activity is from another, and ability to view auto laps instead of laps. also allow comparison of more than one activity. also ability to compare activities across the schema of a database. — Compare page now links each (primary-schema) activity to its detail page; a TimeBehindChart plots each activity's running time gap vs the first activity over a shared distance axis; a Laps/Auto-laps toggle with a shared distance control switches the split view; the page supports N activities (up to 8) via `?ids=` (comma-separated); and it can READ activities from additional Postgres schemas in the same DB via schema-qualified ids (`schema:id`), gated by the `DB_COMPARE_SCHEMAS` allowlist. Cross-schema is strictly read-only — all writes stay pinned to `DB_SCHEMA`, and secondary-schema columns render as plain text (no primary-only deep link). Cross-database is out of scope (single connection).
  - [x] show cumulative pace for laps and auto laps, to the right of cumulative distance. it should use cumulative time and cumulative distance
  - [x] ensure act cal home page and report page use adjusted distance column — The report route (`report.ts`) now returns the activity-level `adjusted_distance`/`adjusted_duration` (the user-editable totals already shown on the calendar dialog and details page) instead of raw `session.total_distance`/`total_timer_time`. For multisport activities the adjusted total is split across legs in proportion to each session's share of the raw session totals (falling back to timer-time share, then equal split), so per-sport grouping is preserved and summing the legs reproduces the activity's adjusted total. Single-session activities get the full adjusted value. The calendar home page was already consistent (its dialog reads `activity.distance` = `adjusted_distance`).
  - [x] remove mac keybinds tooltips — Removed the `⌘C`/`⌘V` `<kbd>` hints from the LapTable Copy/Paste-intensity buttons. The Ctrl/Cmd keyboard shortcuts still work; only the Mac-glyph on-screen labels were dropped.
  - [x] category dropdown does not look good in dark mode — Gave the category `<select>` on the Activity Details page and in `ActivityMetadataEditor` an explicit dark background/border/text plus `[color-scheme:dark]` so the native dropdown popup and its `<option>` list render dark instead of the light OS default.
