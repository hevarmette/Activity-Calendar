...
- [x] **E2E tests with Playwright** — Config + test files covering: calendar navigation, activity detail view + save, search + filter, report grouping.
  ...
  ***
  ...
  - [ ] **Activity search "Select all" button not working** — Toggling "Select all" doesn't persist the selection. `allMatchingIds` is a fresh array reference each render, and the `useEffect([allMatchingIds])` that resets `selectedIds` fires right after the toggle, clearing it. Fix by resetting on a stable key (e.g. joined id string or a committed-filters signature) instead of the array identity. Also reconsider whether the button is needed at all now that per-row checkboxes + "Export all (N)" already cover bulk export — it may be redundant.
  - [ ] **Activity search date field resets while typing the year** — The `type="date"` from/to inputs call `updateParam` on every `onChange`, committing partial/invalid dates to the URL (and resetting `page`), so mid-typing a year (e.g. "202" before "2026") wipes the value. Same class as the previously-fixed calendar-year bug — only commit on a fully valid date (or on blur), ignore invalid intermediate values.
  - [ ] **Allow search with no criteria (browse all)** — `searchApiParams` is `undefined` when no text query is entered, so `useSearch` is disabled and no results render even when only date/sport filters are set or the user just wants to browse. Allow an empty search to fetch the full activity list sorted by activity timestamp descending, then apply the client-side filters/sort on top.
