...
export function computeAutoLaps(
  records: RecordPoint[],
  events: TimerEvent[],
  sport: string,
  lapDistMiles: number
  ...
  for (let i = 0; i < boundaries.length - 1; i++) {
    ...
    let hrSum = 0, hrMax = 0, hrCount = 0;
    let cadSum = 0, cadMax = 0, cadCount = 0;
    for (let j = 0; j < dists.length; j++) {
      if (dists[j]! >= dStart && dists[j]! < dEnd) {
        if (hrs[j]! > 0) { hrSum += hrs[j]!; hrMax = Math.max(hrMax, hrs[j]!); hrCount++; }
        if (cadences[j]! > 0) { cadSum += cadences[j]!; cadMax = Math.max(cadMax, cadences[j]!); cadCount++; }
      }
    }
