...
- [x] **E2E tests with Playwright** — Config + test files covering: calendar navigation, activity detail view + save, search + filter, report grouping.
  ...
  ***
  ...
  - [] fullscreen button for map on the details page.
  - [] before an activity is searched on the details page, the widgets are in a smaller box and after a search the search box is bigger. make them consistent
  - [] the activity report should have a selectable time frame
  - [] the period column should show the start date for the week (on sundays)
  - [] pace per mile should be shown if running is selected and only running.
